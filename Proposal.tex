...
\vspace{-26pt}
  \begin{center}
  \fbox{
    \includegraphics[width=0.48\textwidth]{assets/Annie4xml3}
    }
  \end{center}
\vspace{-20pt}
...
Visual and normal tree-like graph language have different primitives and it is interesting how one can be mapped to the other. When we consider tree-like graphs, there are two structural primitives, namely the nodes, which represent concepts, and the edges, which represent relations between nodes. Additionally we can add semantic primitives like attributes to the concepts and to the relations to show what kind of a relation it is. In normal graphs a relation between two nodes is often quite simple: there is one node that acts as the subject, one node that acts as an object, and the relation that acts like a predicate. Thus we can, for instance, express that the Pythagorean Theorem depends on information about right angles (similar to figure \ref{fig:Annie-context}). These structural primitives and semantic primitives provide the basics that graphs can be built on.\\
...
\vspace{-26pt}
  \begin{center}
  \fbox{
    \includegraphics[width=0.35\textwidth]{assets/Annie3spatial}
    }
  \end{center}
\vspace{-20pt}
