...
\ednote{is relatively shallow, about the implementation we only get to know that it is in Java. An architecture diagram would be helpful, and if you have that, you can explain it. That will give you something to guide the level of presentation.}
...
\item Extract dependencies from relationanl info.
...
For including the XHTML or the PNGs we use self-created boilerplate code, in which only some coordinates and the XHTML or the PNGs for the slide are added. The exact details of this process will be explained in more detail in sections \ref{sec:narrativePaths}, \ref{sec:levels}, \ref{sec:orderedInfoGraphs}, and \ref{sec:RelationalPresentations}. \ednote{check}\\
