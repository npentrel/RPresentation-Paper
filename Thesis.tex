...
As we have pointed out in \autoref{sec:introduction}, presenting information in an effective way is essential to foster learning. This research will answer the following basic questions:

\begin{itemize}
\item How can we combine spatial narrative and semantic closeness of information to improve the presentation of information?
\item Which ways of visualizing information dependencies are there?
\end{itemize}
...
The Oxford Dictionary of English \cite{OED:npentrel14} defines \textit{information} as "what is conveyed or represented by a particular arrangement or sequence of things". Recalling figure %\ref{fig:PythagoreanTheorem}
\ednote{readd figures}
...
Going back to the example of Annie, the information about the Pythagorean theorem that the teacher is just introducing would be considered \textit{course-new}. The information about the right angle which Annie cannot access anymore since she was taught about this too long ago is \textit{course-ancient}. This is marked orange in figure \ref{fig:Annie-ancient}. The information about squares which Annie still remembers is \textit{course-old}. Through the semantic closeness that OMDoc provides, we will be able to determine the status of information within the (dis)course. \ednote{check example and later on refer to this with the different levels.} \\

\subsection{Different Representations of Information}
\label{sec:inforep}
\ednote{Needs a lot of changing. Maybe it won't fit anymore at all.}
...
\ednote{here or rather in 5.3?}

For the implementation of \sys, we chose to use Impress.js, as applications like Prezi do not provide us with an Application Programming Interface (API) that allows the generation of whole presentations. The open-source presentation framework \textbf{Impress.js} \cite{JSImpress:npentrel14} was created by Bartek Szopka's. It combines HTML and CSS3 which makes it highly customizable but unfortunately renders it unsupported by older browsers. The presentation data is entered in the html file; each slide in its own div with data-x, data-y, and data-z attributes to change the position of the slide. The data-x and data-y axis change the position of the element according to the normal x- and y-axes. The data-z attribute changes the closeness of the elements along the z axis.\\
\\
...
Apart from these basic attributes, Impress.js offers the data-scale attribute to scale content, making slides appear bigger or smaller, and the data-rotate attribute which rotates slides in 3 dimensions using data-rotate-x, data-rotate-y, and data-rotate-z. As shown in \autoref{fig:Rotate} this allows the dependency graph to go into the third dimension.\\

In the following code snippets, a few other basic functionalities we will be using are explained.\ednote{cite http://www.andismith.com/blog/2012/01/impress-with-impress/} 
The opacity attribute allows us to discriminate between active and inactive slides which will later on allow us to hide content so as to not throw too much information at a user. Adding an overview can be accomplished with the last past of the CSS snippet and the html snippet below that.\\
...
\end{minted}
\\
To link back to a slide, slides are accessible via IDs. The slide with the ID "conclusion" will thus be accessible by appending "\#/conclusion" to the end of the URL.


\ednote{check link} %http://www.1stwebdesigner.com/creating-stunning-visualizations-with-impress-js/#/slide1}
...
With the OMDoc framework and its tools it is possible to write down a paper in stex. From this paper we can then, in theory, seamlessly create slides both in pdf format as well as in xhtml format. Using a test paper that is supposed to explain the pythagorean theorem and related topics, we can thus create an XML information graph out of the annotated set of documents.\\
...
\subsection{Using Impress.js to Combine OMDoc and Spatial Narration}
\label{sec:wp1}
...
This horizontal stream of slides is achieved by giving the slides incremental data-x values in the attributes. If a slide has dependencies then these will be added as slides below the slide itself. This, in turn, is achieved by keeping the data-x value but increasing the data-y value. Thus users can now simply press down and explore all the dependent information.\\ 
...
\label{sec:wp4}
\ednote{Do I need this or is it okay if I explain it on the go as I do so far?}
...
\ednote{e.g. need to test this via qualitative analysis}
