...
For including the XHTML or the PNGs we use self-created boilerplate code, in which only some coordinates and the XHTML or the PNGs for the slide are added. The exact details of this process will be explained in more detail in subsection \ref{sec:RelationalPresentations}. To complement the HTML, a CSS stylesheet is added which adds the Jacobs logo. The design of the template is based on a presentation by Sebastian Wolf's \cite{Wolf:npentrel15}. \ednote{check subsections}\\
...
\ednote{how to add a slide!}
by parsing the XHTML the created from the OMDocs which is then parsed and added to a presentation file. Each xhtml file represents its own slide and is hence put into its own div as shown in section \ref{sec:Impressjs}. \ednote{add picture of this}\\
...
With the information about the order and the dependencies of information within the paper, \sys automatically creates a presentation. It does this incrementally. As outlined in \autoref{sec:narrativePaths} and visible in \autoref{fig:primaryNarrativePath}, \sys creates the primary narrative path from the order of the slides. After adding each slide it checks whether there is \textit{course-ancient} information, i.e. whether there are dependencies, for that slide. If there are dependencies those are added below the slide itself (see \autoref{fig:visualDependency}). This is achieved by keeping the \texttt{data-x} value but increasing the \texttt{data-y} value. \ednote{code snippet}\\
...
Combining this information with the order in which these pieces of information occur, i.e. with narrative paths and levels, we now have an ordered information graph out of which we can build a presentation that Annie can interact with to learn about the pythagorean theorem. We will look at this graph in the presentation in \autoref{sec:RelationalPresentations}.
...
\vspace{-26pt}
  \begin{center}
  \fbox{
      \includegraphics[width=0.47\textwidth]{assets/final/VisualDependency}
    } 
\vspace{-5pt}
...
\ednote{meaning of moving right and down.}
\ednote {path of going through the slides - identify didactic things!!}
...
\ednote{add example in slides and explain the different levels again}
\ednote {explain ids of slides}
\ednote{refer back to primitives: However, the relations can be much more expressive within impress.js.}


%impress.js gives us all the necessary primitives that were outlined in section \ref{sec:primitives}.  
